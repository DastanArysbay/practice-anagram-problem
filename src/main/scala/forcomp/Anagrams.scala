...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toList.groupBy((c:Char) => c.toLower)
                                                .map{case x->xs => x->xs.length}.toList
                                                .sortBy(_._1)
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.map(word => (wordOccurrences(word),word)).groupBy(_._1)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.map(z => (z._1, z._2 - y.find(_._1 == z._1).getOrElse(('_', 0))._2)).filterNot{case (_,n) => n==0}
